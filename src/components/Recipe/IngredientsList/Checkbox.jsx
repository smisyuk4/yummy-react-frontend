...
export const Checkbox = ({ ingredient, currentShopList, recipeId }) => {
  ...
  useEffect(() => {
    if (currentShopList.length > 0) {
      const inShoppingList = currentShopList.find(
        item => item.ingredientId === id && item.recipeId === recipeId
      );
  ...
  const checkboxChange = event => {
    if (checked) {
      deleteIngredientInShoppingList(shoppingId._id).then(() =>
        setChecked(false)
      );
    } else {
      addIngredient({ ingredientId: id, ttl, thb, measure, recipeId }).then(
        () => setChecked(true)
      );
    }
  };
