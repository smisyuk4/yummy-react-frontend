...
export const MainRecipes = () => {
	...
	const loadMoreButtonRef = useRef(null); //не вызывает повторный рендеринг кнопки.
	...
	//loadMoreButtonRef.current.scrollIntoView делает скролл после нажатия кнопки, чтобы скроллилось при загрузке к нужному елементу, на елемент добавил ref={loadMoreButtonRef}! 
	const loadMore = () => {
		...
		<Section >
			<ul >
				{recipes.slice(0, displayedRecipesCount).map(oneRes => {
					...
					return (
						<li key={uuidv4()} ref={loadMoreButtonRef}>
